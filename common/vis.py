...
class Visualizer:
    ...
    @classmethod
    def visualize_prediction_batch(cls, spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, batch_idx, iou_b=None,pseudo_mask_b=None):
        ...
        if pseudo_mask_b is not None:
            pseudo_mask_b= utils.to_cpu(pseudo_mask_b)

        for sample_idx, (spt_img, spt_mask, qry_img, qry_mask, pred_mask, cls_id,pseudo_mask) in \
                enumerate(zip(spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b,pseudo_mask_b)):
            iou = iou_b[sample_idx] if iou_b is not None else None
            cls.visualize_prediction(spt_img, spt_mask, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, True, iou,pseudo_mask=pseudo_mask)
    ...
    @classmethod
    def visualize_prediction(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, label, iou=None,pseudo_mask=None):
        ...
        if pseudo_mask is not None:
            pseudo_mask = cls.to_numpy(pseudo_mask, 'mask')
            pseudo_mask_pil = Image.fromarray(cls.apply_mask(qry_img.astype(np.uint8), pseudo_mask.astype(np.uint8), pred_color))
            merged_pil = cls.merge_image_pair_vertical(spt_masked_pils + [pseudo_mask_pil,pred_masked_pil, qry_masked_pil])
        else:
    ...
    def merge_image_pair(cls, pil_imgs):
        ...
        return canvas
    @classmethod
    def merge_image_pair_vertical(cls, pil_imgs):
        r""" Horizontally aligns a pair of pytorch tensor images (3, H, W) and returns PIL object """
