...
def test(model, dataloader, nshot):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        pred_mask,pseudo_mask = model.module.predict_mask_nshot(batch, nshot=nshot)
        ...
        if Visualizer.visualize:
            Visualizer.visualize_prediction_batch(batch['support_imgs'], batch['support_masks'],
                                                  ...
                                                  pred_mask,
                                                  batch['class_id'], idx,
                                                  area_inter[1].float() / area_union[1].float(),pseudo_mask_b=pseudo_mask)
...
if __name__ == '__main__':
    ...
    model = PCAS(args.backbone, args.use_original_imgsize)
    model.eval()
